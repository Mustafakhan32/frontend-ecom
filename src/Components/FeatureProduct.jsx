...
const FeatureProduct = () => {
    ...
    const fetchFeaturedProducts = async () => {
        try {
            const response = await axios.get(`${localstring}/api/product/get-featured-product`);
    ...
    return (
        <div className="p-0 mb-4 mt-8 container mx-auto" id="featured-products" >
            <h1 className="text-3xl font-sans text-center mb-3">FEATURED PRODUCTS</h1>
            {loading && <p className="text-center font-semibold text-xl">Loading Featured Products...</p>}
                    ...
                    return (
                        <div key={product._id} className="w-full max-w-[380px] mx-auto sm:max-w-none rounded-lg overflow-hidden shadow-lg bg-white">
                            <div className="relative">
                                ...
                                <div className="absolute bottom-0 left-0 p-4 flex flex-col items-start space-y-2">
                                    ...
                                    <img
                                        ...
                                        src={wishlist}
                                        className={`w-8 text-black p-0.5 rounded-full shadow-md hover:text-white active:text-white active:bg-black hover:bg-black transition-colors cursor-pointer duration-300 ease-in-out ${isInWishlist ? "bg-red-500" : "hover:bg-black"}`}
                                    />
                                    <img
                            ...
                            <div className="p-4 py-2 bg-gray-100" title="Product-Card">
                                ...
                                <div className="w-32">
                                    <Link to={`/category/${product?.category?.slug}`} key={product?.category?._id} title="Categories" className="hover:underline text-black">
